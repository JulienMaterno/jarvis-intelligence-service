...
async def trigger_sync(sync_type: str) -> bool:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            response = await client.post(url, headers=_get_sync_headers(), timeout=30.0)
...
async def trigger_quick_sync(entity_type: str, hours: int = 1) -> dict:
    ...
    try:
        ...
        response = await client.post(url, params=params, headers=_get_sync_headers(), timeout=30.0)
